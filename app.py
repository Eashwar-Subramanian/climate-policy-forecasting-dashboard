...
def validate_model(ts_data):
    if len(ts_data) < 104:  # Need at least 2 years
        return None
    train = ts_data[:-52]
    test = ts_data[-52:]
...
def suggest_policies(location, forecast_df, hist_data):
    policies = []
    mae = validate_model(hist_data)
    if mae is not None:
        policies.append(f"SARIMA MAE for {location}: {mae}°C (optional validation).")
...
def temporal_map():
    ...
    m = Map(location=loc_coords, zoom_start=6)
